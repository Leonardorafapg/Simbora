...
export async function exportCalendarToPdf(
  ...
  doc.setFontSize(10.65); // 15 * 0.710125 — 35% menor, +15%, -5% sobre isso
  doc.setTextColor(20, 20, 20);
  doc.text("CRONOGRAMA DE POSTAGEM", pageWidth / 2, y, { align: "center" });
  y += 20;

  doc.setFont("helvetica", "normal");
  doc.setFontSize(7.81); // 11 * 0.710125
  ...
  function drawHeaderRow() {
    ...
    doc.setFont("helvetica", "bold");
    doc.setFontSize(7.10); // 10 * 0.710125
  ...
  for (const entry of ordenados) {
    doc.setFont("helvetica", "normal");
    doc.setFontSize(6.39); // 9 * 0.710125
