...
const steps = [
  {
    title: "Manda seu @ do Instagram",
    description: "Você envia o @ do seu perfil pelo WhatsApp — leva menos de 1 minuto.",
  },
  {
    title: "Nossa equipe analisa",
    description:
      "Avaliamos posicionamento, qualidade de vídeo e conformidade com a Resolução CFM 2.336/2023.",
  },
  {
    title: "Retorno em até 24h úteis",
    description:
      "Você recebe o diagnóstico completo direto no WhatsApp, com sugestões práticas pra aplicar já.",
  },
];
...
export default function ComoFunciona() {
  ...
  useGSAP(
    () => {
      const items = gsap.utils.toArray<HTMLElement>(".como-funciona-step");
      gsap.fromTo(
        items,
        { opacity: 0, y: 32 },
        {
      ...
      <div className="max-w-[1100px] mx-auto">
        <div className="text-center mb-16 max-sm:mb-10">
          ...
          <h2 className="font-sans font-normal text-[3.2rem] max-sm:text-3xl text-dark leading-[1.2]">
            Do @ ao diagnóstico, em 3 passos simples.
          </h2>
        </div>
        ...
        <div className="grid grid-cols-3 gap-8 max-md:grid-cols-1">
          {steps.map((step, idx) => (
            <div
              key={idx}
              className="como-funciona-step opacity-0 bg-white border border-gray-200 rounded-md p-8"
            >
